...
**相关实现入口**：大屏载荷 `backend/backoffice/display_services.py::get_screen_payload`；排产 MySQL `backend/backoffice/schedule_mysql_source.py`；能耗看板 `backend/backoffice/energy_dashboard_services.py`；能耗子页前端 `frontend/src/EnergyDashboardBoard.jsx`（请求 `/api/energy-dashboard`）。
...
| 产量执行概览、近 8 小时趋势 | **默认 Mock / 快照** | `ProductionSnapshot` 仅由 `python manage.py load_mock_screen_data`（及首次访问时的 mock 引导）写入；**无独立采集任务向产量快照灌真实产量**。 |
...
2. **右屏真实排产**：后台维护产线 `lineCode` 与外部 `machines.lineCode` 一致，并配置 `schedule` 子页绑定或 `DB_003` 数据库源。
