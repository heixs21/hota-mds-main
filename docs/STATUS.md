...
- 产量执行概览、近 8 小时产量趋势、左屏嵌入的区域能耗概览（`ProductionSnapshot` / `EnergySnapshot`，主要由 `load_mock_screen_data` 写入）。
- 未配置排产库时的右屏甘特。
...
- 前端仍不得直连外部系统。
- 产量/趋势/左屏嵌入能耗等仍以快照 mock 为主，完整 M5 产量与能耗快照自动化采集待对接。
...
- 更新本文档第 0 节、`README.md`、`DOCS_OVERVIEW.md`，与当前代码行为对齐（右屏 MySQL 甘特、OPC UA 监控、能耗看板 API、产量/嵌入能耗仍为快照 mock 等）。
